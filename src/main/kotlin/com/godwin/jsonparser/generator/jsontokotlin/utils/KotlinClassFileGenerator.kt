...
import com.godwin.jsonparser.generator.common.filetype.KotlinFileType
    ...
    private fun generateKotlinClassFile(
        ...
        val file =
            psiFileFactory.createFileFromText(
                "${fileName.trim('`')}.kt", KotlinFileType(), kotlinFileContent, LocalTime.now().toEpochSecond(
                    LocalDate.now(ZoneOffset.UTC),
                    ZoneOffset.UTC
                ), // Dummy physical timestamp for mimic it is an actual file
                true
            )
