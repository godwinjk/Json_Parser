...
interface IConfigManager {
    ...
    //Dart Configs
    private val IS_DART_VAR_FINAL: String
        get() = "jsonParser_dart_final"
    private val IS_DART_NULLABLE: String
        get() = "jsonParser_dart_nullable"
    private val TARGET_JSON_CONVERTER_LIB_KEY_DART: String
        get() = "target_json_converter_lib_key_dart"
...
//Dart configuration
    var isPropertyFinal: Boolean
        ...
        set(value) = if (isTestModel) {
            ...
            PropertiesComponent.getInstance().setValue(IS_DART_VAR_FINAL, value)
        }
    var isPropertyNullable: Boolean
        ...
        set(value) = if (isTestModel) {
            ...
            PropertiesComponent.getInstance().setValue(IS_DART_NULLABLE, value)
        }

    var targetJsonConverterDart: TargetJsonConverter
        get() = if (isTestModel) TestConfig.targetJsonConvertLib else {
            val value = PropertiesComponent.getInstance().getValue(TARGET_JSON_CONVERTER_LIB_KEY_DART)
            //Next step try to keep compatible with 3.5.1 and before version of plugin,
            //Please see : https://github.com/wuseal/JsonToKotlinClass/issues/284
            val compatibleValue = if (value =="Serilizable") "Serializable" else value
            try {
        ...
        set(value) = if (isTestModel) {
            ...
            PropertiesComponent.getInstance().setValue(TARGET_JSON_CONVERTER_LIB_KEY_DART, value.name)
        }
